...
'''
#imports
import sys, os, glob, itertools, csv, pp, time, gensim.corpora, gensim.models, gensim.similarities, MySQLdb, nltk, string, gc, urlparse, resource, caulk, pprint
caulk.install()
...
def calculateSimilarity(path,fileName,originalContent, originalId,category,depth,limit,operationType = "3"):
    ...
    if not os.path.isfile(resultsSavePath):
        ...
        #print gensim data
        print fileName, corpus, dictionary, tfidfModel
        ...
        #create dict, sort, filter, write to either db or csv
            ...
            #print type(index), len(index), index
            #print fileName
            #print dictionary, corpus, tfidfModel
            ...
            for descriptionLevel, idLevel in  itertools.izip(originalContent,originalId):
                ...
                for sim in sims:
                    print "sim:\t","\t",sim,"\t",type(sim[0]),"\t",type(sim[1])
                    if sim[0] in dictAnalysis:
                        dictAnalysis[sim[0]]['nrOcc'] += 1
                        dictAnalysis[sim[0]]['sim']+=sim[1]
                    else:
                        originalIDItem=reader[str(sim[0])]
                        print "OID:\t",sim[0],"\t",type(sim[0]),originalIDItem,"\t",type(originalIDItem)
                        dictAnalysis[sim[0]] = {'category': category, 'depth': depth, 'idLevel': idLevel, 'ocID': int(originalIDItem), 'sim':sim[1], 'nrOcc': 1}
...
def returnSimilaritiesCategory(category, compareTo="3", limit = "100"):
    ...
    myFile= open( "memoryAnalyzer.txt", "wb") 
    #sys.stdout= myFile
    ...
    for testingDataItem in testData:
        ...
        for depth in ranger:
            ...
            #if nr of returned > limit
            if len(sqlRandomResults)>limit:
                indices = random.sample(range(len(sqlRandomResults)), 100)
                sqlRandomResults=[sqlRandomResults[i] for i in indices]
            ...
            path = "test1/"+str(testingDataItem)+"/"
