...
if($argv[0]){
 ...
 $short_tags = array(
  ...
  'catid' => '<tag catid=',
  'narrow' => '<narrow r:resource=',
  'narrow1' => '<narrow1 r:resource=',
  'narrow2' => '<narrow2 r:resource=',
  'symbolic' => '<symbolic r:resource=',
  'symbolic1' => '<symbolic1 r:resource=',
  'symbolic2' => '<symbolic2 r:resource=',
  'newsGroup' => '<newsGroup r:resource=',
  'related' => '<related r:resource=',
  'editor' => '<editor r:resource=',
  'letterbar' => '<letterbar r:resource=',
  'altlang' => '<altlang r:resource='
 );
 ...
 //----- find content between tags
  while (list ($key, $val) = each ($open_tags)){
   if(eregi($open_tags[$key]."([^<]{1,})".$close_tags[$key], $data,$content)){
    ...
    store_data($closeThisContainer[0],$closeThisContainer[1],$key,$content[1],$id,"structure");
   }
  }
 ...
 function run_parser(){
  ...
  $info = get_file_info("1",STRUCTURE_RDF,"",$start_time,"");
  $filesize = $info[0];
  $benchmark = $info[1];
  $adj_filesize = $info[2];

  if(ECHO_STRUCTURE_STATS){
  ...
  $createtable = suckDMOZ_structure_dbconnect($fields,"create");
  if($createtable[0]=="error"){
   if($createtable[2]=="No Database Selected"){die("Database Error, Could Not Create 'DMOZ_structure'");}
   else{echo $createtable[2]."\n";}
  }

  if (!($fp = fopen(STRUCTURE_RDF, 'r'))) {
     die("Could not open ".constant("STRUCTURE_RDF")." for parsing!\n");
  }
  ...
  while (!feof ($fp)) {
   ...
   if($next_benchmark>$benchmark){
    $next_benchmark=0;
    $info2 = get_file_info("2",STRUCTURE_RDF,$eaten,$start_time,$count_rows);
    $echo ="REMAINING: {".$info2[0]."} (".$info2[1]."%) [".$info2[2]."]";
    $echo.="\n[APPROX TOTAL RUN TIME: ".$info2[3]."]";
    $echo.="\n[APPROX TOTAL ROWS: ".$info2[4]."]";
    $echo.="\n[APPROX MPH: ".$info2[5]."]";
    if(ECHO_STRUCTURE_STATS){echo_status($start_time,$count_rows,$echo);}
   }
  }

  fclose($fp);

  if(ECHO_STRUCTURE_STATS){echo_status($start_time,$count_rows,"FINISHED");}
 ...
 run_parser();
	...
	echo "This script must be run from the command line: <br><br>php -q ".constant("SUCKDMOZ_DIR")."/suckDMOZ_structure_parser.php  > ".constant("SUCKDMOZ_DIR")."/DMOZ_structure_parser_result.txt &";
