...
while($rowSID = mysql_fetch_array($rezSid)){
	...
	$sqlListActions = "SELECT sessionID, Window, actionTaken, COUNT(actionTaken), userID FROM  `actiontimes` 
						where sessionID = '".$rowSID[sessionID]."' group by Window, actionTaken;";
	...
	while ($rowListActions = mysql_fetch_array($rezListActions)){
		...
		$sqlInsertLA = "insert into actionLink (sessionID, Window, actionTaken, nrActionTaken,userID) 
						values ('".$rowListActions [0]."','".addslashes($rowListActions[1])."','".$rowListActions[2]."','".$rowListActions[3]."','".$rowListActions[4]."')";
