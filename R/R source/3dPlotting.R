...
for (i in 1:20){
	...
	userSubset<-subset(user01,userID == sessionU01[i,],select=c(timelineMSEC,X,Y))
	userSubset
	x<-as.numeric(userSubset$X)
	y<-as.numeric(userSubset$Y)
	scatterplot3d(userSubset$X,userSubset$Y,userSubset$timelineMSEC, pch=16, highlight.3d=TRUE, type="h", main="Active screen regions per user")
