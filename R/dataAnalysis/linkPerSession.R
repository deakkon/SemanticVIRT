...
#data for user 1
mycon <- dbConnect(MySQL(), user='root',dbname='test',host='localhost',password='')
rsActionTaken <- dbSendQuery(mycon, "select userID, sessionID, nrLinksVisited from nrvisitedlinks")
...
#plot data for each session
	...
	for (i in 1:20) {	
		...
		print(names[i,1])
		x<-subset(actionTaken,userID == names[i,],select=c(sessionID,nrLinksVisited))
		x[2]
		x
		i
		tmp=as.vector(as.matrix(x[2]))
		tmp
		class(tmp)
		print(tmp)
		name = names[i,]
		name
		if(i==1)
			plot(1:length(tmp),cumsum(tmp), type="l", main=name,ylim=c(0,420))
		else
