...
mycon <- dbConnect(MySQL(), user='root',dbname='test',host='localhost',password='')
rsNames <- dbSendQuery(mycon, "select distinct sessionID from actionsession where userID = 'ID01'")
...
#data for user 1
mycon <- dbConnect(MySQL(), user='root',dbname='test',host='localhost',password='')
rsActionTaken <- dbSendQuery(mycon, "select sessionID, actionTaken,numberTimes from actionsession where userID = 'ID01'")
...
#plot data for each session
	...
	for (i in 1:10) {	
		...
		x<-subset(actionTaken,sessionID == names[i,],select=c(actionTaken,numberTimes))
		...
		plot(1:dim(x)[1],action,"p",xlab=tmp)
		tmp1=1:num
		tmp2=rep("=",num)
